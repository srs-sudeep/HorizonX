...
export const authApi = {
  ...
  login: async (email: string, password: string): Promise<{ user: User; accessToken: string; refreshToken: string }> => {
    ...
    // Generate tokens
    const { accessToken, refreshToken, accessTokenExpiry, refreshTokenExpiry } = generateTokens(user, defaultRole);
  ...
  refreshToken: async (refreshToken: string): Promise<{ accessToken: string; refreshToken: string }> => {
    ...
    // Generate new tokens
    const { accessToken, refreshToken: newRefreshToken, accessTokenExpiry, refreshTokenExpiry } = generateTokens(user, currentRole);
  ...
  // Change user role
  changeRole: async (accessToken: string, newRole: UserRole): Promise<{ accessToken: string; refreshToken: string }> => {
    await delay(500);
    ...
    const { accessToken: newAccessToken, refreshToken: newRefreshToken, accessTokenExpiry, refreshTokenExpiry } = generateTokens(user, newRole);
  ...
  register: async (name: string, email: string, password: string): Promise<{ user: User; accessToken: string; refreshToken: string }> => {
    // Simulate network delay
    await delay(100000);
    ...
    const { accessToken, refreshToken, accessTokenExpiry, refreshTokenExpiry } = generateTokens(newUser, defaultRole);
