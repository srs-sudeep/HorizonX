...
export const getFilteredModules = (userRoles?: UserRole[]) => {
  ...
  return getSortedModules().filter(
    module => !module.requiredRoles || module.requiredRoles.some(role => userRoles.includes(role))
  );
};
...
export const getFilteredSubModules = (moduleId?: string, userRoles?: UserRole[]) => {
  ...
  return getSortedSubModules(moduleId).filter(
    subModule =>
