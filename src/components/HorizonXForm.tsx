                                ...
                                {(() => {
                                  const filteredOptions =
                                    field.options?.filter(opt => {
                                      const searchTerm =
                                        searchTerms[field.name]?.toLowerCase() || '';
                                  ...
                                  const currentSelectedValues = Array.isArray(formData[field.name])
                                    ? formData[field.name]
                                    : [];

                                  const filteredOptionValues = filteredOptions.map(opt =>
                                    String(typeof opt === 'string' ? opt : opt.value)
                                  );

                                  const isAllSelected =
                                    filteredOptionValues.length > 0 &&
                                    filteredOptionValues.every(value =>
                                      currentSelectedValues.map(String).includes(value)
                                    );

                                  const isPartiallySelected =
                                    filteredOptionValues.some(value =>
                                      currentSelectedValues.map(String).includes(value)
                                    ) && !isAllSelected;
                                  ...
                                  return filteredOptions.length > 0 ? (
                                    <>
                                      <div
                                        className={cn(
                                          'relative flex cursor-pointer select-none items-center rounded-md px-2 py-2 text-sm outline-none transition-colors border-b border-border mb-2',
                                          'hover:bg-muted hover:text-muted-foreground font-medium'
                                        )}
                                        onClick={() => {
                                          let updatedFormData;
                                          if (isAllSelected) {
                                            // Deselect all filtered options
                                            const remainingValues = currentSelectedValues.filter(
                                              (val: string) =>
                                                !filteredOptionValues.includes(String(val))
                                            );
                                            updatedFormData = {
                                              ...formData,
                                              [field.name]: remainingValues,
                                            };
                                          } else {
                                            // Select all filtered options
                                            const newValues = [
                                              ...currentSelectedValues,
                                              ...filteredOptionValues.filter(
                                                val =>
                                                  !currentSelectedValues.map(String).includes(val)
                                              ),
                                            ];
                                            updatedFormData = {
                                              ...formData,
                                      ...
                                      >
                                        <div className="flex items-center gap-2 w-full">
                                          <div
                                            ...
                                            ref={ref => {
                                              if (ref && isPartiallySelected && !isAllSelected) {
                                                // Create an indeterminate state visual
                                                ref.innerHTML =
