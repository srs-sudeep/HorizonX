...
export const HorizonXTable: React.FC<HorizonXTableProps> = ({
  data,
  customRender = {},
  isLoading = false,
  filterConfig = [],
  ...
  const [dateRangeTemp, setDateRangeTemp] = useState<Record<string, { from?: Date; to?: Date }>>(
    {}
  );
  const [multiSelectTemp, setMultiSelectTemp] = useState<Record<string, string[]>>({});
  const [multiSelectPopoverOpen, setMultiSelectPopoverOpen] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    // Sync multiSelectTemp with columnFilters when popover is closed
    Object.keys(multiSelectPopoverOpen).forEach(column => {
      if (!multiSelectPopoverOpen[column]) {
        setMultiSelectTemp(prev => ({
          ...prev,
          [column]: Array.isArray(columnFilters[column]) ? columnFilters[column] : [],
        }));
      }
    });
  }, [columnFilters, multiSelectPopoverOpen]);
  ...
  const renderFilter = (filter: FilterConfig) => {
    ...
    switch (filter.type) {
      case 'multi-select': {
        ...
        const isAllSelected = allOptions.length > 0 && selectedValues.length === allOptions.length;
        const isPartiallySelected =
          selectedValues.length > 0 && selectedValues.length < allOptions.length;

        const tempSelected = multiSelectTemp[filter.column] ?? selectedValues;
        const popoverOpen = multiSelectPopoverOpen[filter.column] ?? false;
        ...
        return (
          ...
          >
            <Popover
              open={popoverOpen}
              onOpenChange={open => {
                setMultiSelectPopoverOpen(prev => ({
                  ...prev,
                  [filter.column]: open,
                }));
                if (!open) {
                  // Only apply filter when popover closes
                  if (JSON.stringify(tempSelected) !== JSON.stringify(selectedValues)) {
            ...
            >
              ...
              <PopoverContent className="w-56 p-2">
                ...
                {allOptions.length > 0 && (
                  <div
                    className="flex items-center gap-2 py-2 px-2 rounded-md hover:bg-muted cursor-pointer border-b border-border mb-2"
                    onClick={() => {
                      setMultiSelectTemp(prev => ({
                        ...prev,
                        [filter.column]: isAllSelected ? [] : allOptions,
                      }));
                    }}
                  >
                    <Checkbox
                ...
                <ScrollArea className="max-h-48 overflow-auto">
                  {allOptions.map(opt => (
                    <div
                      ...
                      onClick={() => {
                        const exists = tempSelected.includes(opt);
                        setMultiSelectTemp(prev => ({
                          ...prev,
                          [filter.column]: exists
                            ? tempSelected.filter((v: string) => v !== opt)
                            : [...tempSelected, opt],
                        }));
                      }}
                    >
                ...
                {tempSelected.length > 0 && (
                  <div className="pt-2 mt-2 border-t border-border">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="w-full text-sm hover:bg-destructive/10 hover:text-destructive"
                      onClick={() =>
                        setMultiSelectTemp(prev => ({
                          ...prev,
                          [filter.column]: [],
      ...
      case 'date-range': {
        const currentValue =
          filter.value !== undefined ? filter.value : columnFilters[filter.column];
        const tempRange: { from?: Date | undefined; to?: Date | undefined } = dateRangeTemp[
          filter.column
        ] ||
          currentValue || { from: undefined, to: undefined };
        ...
        return (
          ...
          >
            <DateRangePicker
              value={{ from: tempRange.from, to: tempRange.to }}
              onChange={range => {
                // Defensive: Only handle if range is an object with 'from'/'to'
                if (
                  !range ||
                  typeof range !== 'object' ||
                  (!('from' in range) && !('to' in range))
                ) {
                  return;
                }
                const { from, to } = range as { from?: Date; to?: Date };
                setDateRangeTemp(prev => ({
                  ...prev,
                  [filter.column]: {
                    from,
                    to,
