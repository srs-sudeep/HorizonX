    ...
    const searchInSubModules = (items: HierarchicalSubModule[]): HierarchicalSubModule[] => {
      ...
      for (const item of items) {
        ...
        const childMatches = item.children 
          ? searchInSubModules(item.children) 
          : [];
      ...
      <div key={subModule.id} className="mb-1">
        ...
        >
          ...
          {typeof subModule.badge === 'number' && subModule.badge > 0 && (
            <Badge variant="default" className="ml-2 bg-sidebar-primary text-sidebar-primary-foreground">
              {subModule.badge}
            </Badge>
          )}
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isActive ? 'bg-sidebar-accent text-sidebar-accent-foreground' : 'hover:bg-sidebar-accent/10 text-sidebar-foreground/70',
          ...
          <ScrollArea className="flex-1">
            {isLoading ? (
              ...
              <div className="py-3 px-2">
                {activeModule && getFilteredBySearchSubModules().map(subModule => 
                  ...
                  <>
                    {Object.entries(subModulesMap)
                      ...
                      .flatMap(([moduleId, subModules]) => {
                        const filteredSubModules = subModules.filter(sm =>
                        ...
                        return filteredSubModules.length > 0 ? (
                          <div key={moduleId} className="mt-4 pt-4 border-t border-sidebar-border/50">
